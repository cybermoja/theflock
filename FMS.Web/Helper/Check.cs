...
namespace FMS.Web.Helper
{
    public class Check
    {
        ...
        public class Argument
        {
            ...
            [DebuggerStepThrough]
            public static void IsReferrerKey(string argument, string argumentName)
            {
                IsNotEmpty(argument, argumentName);

                const string pattern = "^BAR[0-9]{5}C|R$";

                if (!Regex.IsMatch(argument, pattern))
                {
                    throw new ArgumentException("\"{0}\" is not a valid referral key.".FormatWith(argumentName), argumentName);
                }
            }
        }
    }
